...
namespace Munzevi\Passcon\Rules;

use Illuminate\Contracts\Validation\Rule;
use Illuminate\Support\Str;

/*
* borrowed from laravel/fortify package and slightly edited to feeding from config file.
* all respects original creators of the rules. you may see original rules at fortify
* repository which address is https://github.com/laravel/fortify/blob/1.x/src/Rules/Password.php
*/
...
class Password implements Rule
{
    ...
    /**
     * Get setted minimum password length value from config and set it
     *
     * @param  int  $length
     * @return $this
     */
    ...
    {
        $this->length = config('passcon.length', true);
    ...
    {
        $this->requireUppercase = config('passcon.upperCase', true);
    ...
    {
        $this->requireNumeric = config('passcon.numeric', true);
    ...
    {
        $this->requireSpecialCharacter = config('passcon.special', false);
    ...
    {
        $this->message = config('passcon.message');
